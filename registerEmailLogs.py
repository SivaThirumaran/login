...
class registerpage(unittest.TestCase):
    ...
    def test_register():
        ...
        driver.rows=XLutils.getRowCount(path ,'Email')
        ...
        for r in range(2, driver.rows + 1):
            ...
            driver.FirstName = XLutils.readData(path, 'Email', r, 1)
            driver.LastName = XLutils.readData(path, 'Email', r, 2)
            driver.EmailAddress = XLutils.readData(path, 'Email', r, 3)
            driver.PassWorD = XLutils.readData(path, 'Email', r, 4)
            driver.PhoneNumber = XLutils.readData(path, 'Email', r, 5)
            driver.exp = XLutils.readData(path, 'Email', r, 6)
            ...
            try:
                ...
                S.screenshot(driver)
                time.sleep(2)
                Email = driver.find_element_by_xpath("//*[@class='fild_error']")
                Email_message = Email.text
                time.sleep(2)
                U.log("WARNING", Email_message + str(driver.EmailAddress), log_file)
                time.sleep(2)
